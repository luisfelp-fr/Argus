...
def render_seasonal_mode() -> None:
    ...
    with st.expander("ℹ️ Glossário — entenda cada modelo e análise"):
        st.markdown(
            "#### 🤖 Modelos preditivos\n"
            "- **RandomForest (Floresta Aleatória)** — treina **muitas árvores de decisão**, cada "
            "uma vendo uma amostra e um subconjunto diferente das variáveis, e combina as previsões "
            "pela **média**. Captura relações **não lineares e interações** entre indicadores, exige "
            "pouco ajuste e é **robusto a ruído e a sobreajuste** — a opção mais **estável** para "
            "começar. Costuma ser um pouco menos preciso que o boosting e não extrapola além da "
            "faixa observada nos dados.\n"
            "- **XGBoost (Gradient Boosting)** — treina árvores **em sequência**, cada nova árvore "
            "corrigindo o **erro** deixado pelas anteriores, com **regularização** para evitar "
            "sobreajuste. Em dados de tabela costuma ser o **mais preciso**, mas é **mais sensível** "
            "ao ajuste de parâmetros e precisa de uma quantidade razoável de dados. Aparece como "
            "opção apenas quando a biblioteca está instalada.\n"
            "\n"
            "#### 📊 Análises estatísticas de associação\n"
            "- **Pearson r** — correlação **linear** entre a variável e o indicador sazonal "
            "(−1 a +1; 0 = sem relação linear). Simples e direta, mas sensível a valores extremos.\n"
            "- **Spearman r** — correlação de **ordem (ranking)**; capta relações **monotônicas** "
            "(sobe-sobe ou sobe-desce) mesmo que **não sejam lineares** e é **mais robusta a "
            "outliers** (−1 a +1). É o critério principal de ordenação do ranking.\n"
            ...
            "(inclusive não linear) entre a variável e o alvo (≥ 0; 0 = independentes). Não indica "
            "o **sentido** (se faz subir ou descer).\n"
            "\n"
            "#### 🧠 Explicabilidade (por que o modelo previu aquilo)\n"
            "- **Importância do modelo** — o quanto cada variável **reduziu o erro** nas árvores do "
            "modelo. Mostra quem o modelo mais **usou**, mas **não** o sentido do efeito.\n"
            "- **SHAP** — atribui de forma **justa** quanto cada variável **empurrou** cada previsão "
            "para cima ou para baixo (teoria dos jogos). O ranking usa o **|impacto| médio**; também "
            "permite ver a contribuição **em um período específico**.\n"
            "\n"
            "#### 🎯 Métricas de desempenho do modelo (medidas no teste)\n"
            "- **MAE** — erro **médio absoluto**, nas unidades do indicador (menor é melhor).\n"
            "- **RMSE** — como o MAE, mas **penaliza mais os erros grandes** (menor é melhor).\n"
            "- **MAPE** — erro médio em **percentual** (menor é melhor).\n"
            "- **R²** — **fração da variação** do indicador explicada pelo modelo "
            "(1 = perfeito; 0 ou negativo = não explica).\n"
            "\n"
            "#### 🧩 Outros conceitos\n"
            "- **Lag (minutos)** — **atraso** entre o processo e o indicador sazonal. O sistema cria "
            "blocos de **períodos anteriores** (lag 0 = período atual) até o máximo definido em "
            "minutos.\n"
            "- **Score consolidado** — combina **Spearman + Informação Mútua + Importância + SHAP** "
            "(normalizados de 0 a 1) num **índice único de evidência** por variável.\n"
            "- **Período sazonal** — janela em que o processo é resumido, definida pelos **horários "
            "do indicador sazonal** (ex.: 1 dia, ou a cada 4 horas)."
        )
