...
- **🏭 Produção Diária (novo):** correlaciona o comportamento dos indicadores de
  processo **minuto a minuto** com a **produção informada uma vez ao dia**. Veja a
  seção *"Modo Produção Diária"* abaixo.
...
## Modo Produção Diária

Transforma os dados minuto a minuto em **variáveis explicativas diárias** e busca
quais indicadores têm **maior evidência estatística de influência** sobre a produção
(prováveis causas — sem afirmar causalidade absoluta).

**Entrada:** um único arquivo Excel com **duas abas** (o usuário seleciona quais):
processo (`DataHora` + indicadores) e produção (`Data` + `Producao`). Em CSV, sem
abas, carregue a produção num segundo upload.

**O que o sistema faz:**
1. **Agrega minuto → dia** por indicador: média, mediana, mín, máx, amplitude, desvio
   padrão, CV, percentis (10/25/75/90), soma, nº de válidos, % de ausentes.
2. **Instabilidade:** Δ médio entre leituras, taxa máxima de variação, nº de
   oscilações bruscas.
3. **Tempo fora de faixa** (limites configuráveis por indicador): minutos abaixo/acima,
   % do dia fora, área acumulada acima/abaixo.
4. **Lag em dias:** cria D0, D-1, …, D-N (até o limite escolhido).
5. **Turnos opcionais:** média/desvio por 00–08, 08–16, 16–24.
6. **Junta com a produção**, limpa a base (colunas constantes, nulos, padroniza nomes).
7. **Estatística:** Pearson e Spearman (com *p-value*) e *mutual information*.
8. **Modelo:** Random Forest (ou XGBoost) com **split temporal** (sem embaralhar) e
   métricas MAE, RMSE, MAPE, R².
9. **Explicabilidade:** importância do modelo e **SHAP**.
10. **Ranking consolidado** (Spearman + MI + importância + SHAP) e **diagnóstico
    gerencial** automático.
11. **Diagnóstico do dia:** produção real, principais contribuintes e comparação com
    a média histórica.
12. **Exporta** todos os resultados num Excel multi-aba.

**Validação rápida:**

```bash
python sample_data_producao.py     # cria sample_producao.xlsx (abas Processo + Producao)
streamlit run app.py               # modo "Produção Diária": suba o arquivo, abas Processo/Producao,
                                   # colunas DataHora/Data/Producao, lag 3, limite de pH [4.8, 5.6]
```

Espera-se que liderem o ranking: `Vazao_*_mean_lag_0d` (direto), `Temperatura_std_lag_0d`
(inverso) e o tempo de `pH` fora de faixa em **D-1** (efeito defasado).
